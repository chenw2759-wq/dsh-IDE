...
- **SSH 远程文件树子目录经常加载不出来/卡顿的根因修复（三层）**：① `SshEngine` 每次文件操作都调用 `client.sftp()` 开一个**新的 SFTP subsystem channel** 且从不关闭，而 OpenSSH 默认 `MaxSessions=10`——长连接池复用下 channel 持续累积，第 11 个起全部报 `Channel open failure: open failed`，远程列举/读取**间歇性失败**。修复：每个池化连接**缓存复用单个 SFTP channel**（`sftpFor()`），连接断开/关闭自动失效缓存。② `classifyEntries` 对**每个 symlink 串行 `sftp.stat`**——在 AutoDL 这类高延迟中转链路上（实测每次 SFTP 操作 0.5~1.7s RTT），conda/venv 的 `bin` 目录几十上百个 symlink 会把一次 ls 拖成**几十秒甚至永久卡住**（实测 39 个 symlink 串行 stat = 7.8s，300 个 ≈ 60s+）。修复：symlink stat **分批并行**（64/批，SSH 窗口内流水线 ≈ 1 个 RTT；实测 192 条目含 39 symlink 的目录 7.8s→3.7s）。③ SFTP 请求**无超时**——网络半死时 `readdir`/`stat` 回调永不触发，Promise 永不 settle，浏览器永久转圈。修复：`ls`/`stat`/`readFile` 加**超时兜底**（15s/15s/60s，超时即报错并触发 `withClient` 断连重连），文件树绝不会无限挂起。真机验证：修复前模拟在第 11 个 channel 起失败（10 成功/8 失败）；修复后同一连接连续 18 次 `ls` 18/18 成功、symlink 密集目录连续 12 次 ls 12/12 成功且单次从串行 7.8s 降到 ~3s（剩余时间主要是高延迟网络的固定 RTT）。
