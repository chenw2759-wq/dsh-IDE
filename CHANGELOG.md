...
- **float 预览预设位置吸附（桌面图标式）**：松手时若预览框中心落在预设锚点 120px 内即吸附——`far-right`（最右居中）/ `cover-tree`（盖住文件树，树自动折叠为圆形按钮）/ `below-tree`（树下方右下角）/ `chat`（聊天区下方左下角）；拖出吸附区即回到自由浮动；吸附区随窗口缩放重算位置（`aionui-float-dock` 持久化）。
