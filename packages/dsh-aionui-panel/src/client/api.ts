...
export class PanelApi {
  ...
  /** Rename / move a path. */
  rename(root: string, path: string, newPath: string): Promise<PanelEnvelope<{ ok: true }>> {
    return post('/aionui-panel/rename', { root, path, newPath })
  }

  /** Copy a path (into the same tree; read + write). */
  copy(root: string, path: string, newPath: string): Promise<PanelEnvelope<{ ok: true }>> {
    return post('/aionui-panel/copy', { root, path, newPath })
  }
