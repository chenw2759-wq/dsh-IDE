...
export function PreviewPanel({ stores }: { stores: PanelStores }): JSX.Element {
  ...
  /** Run the active tab's file (python/script-like extensions). */
  const runActiveFile = (): void => {
      ...
      {activeTab !== null && (
        <>
          ...
          <TabContent
            ...
            onSave={() => void preview.saveTab(activeTab.id)}
            onEditDiffContent={(id, text) => preview.setEditDiffContent(id, text)}
            onSaveEditDiff={(id) => void preview.saveEditDiff(id)}
          />
        </>
      )}
      {activeTab !== null && terminal.open && (
        <TerminalPanel
          root={activeTab.root}
