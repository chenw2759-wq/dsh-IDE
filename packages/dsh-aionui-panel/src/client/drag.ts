...
export function handlePointerDragStart(event: PointerEvent, el: HTMLElement, opts: DragStartOptions): () => void {
  ...
  // The frame is marked `data-aionui-frame` by the layout controller on attach
  // (rc.6 AppFrame has no `data-dsh-frame` attribute to match).
  const frame = el.closest('[data-aionui-frame]')
