 ...
 * the fs/git operations, a command-exec route (local subprocess or remote
 * SSH engine), and one SSE stream (fs changes + git status changes) per
 * project root. The services own gating and parsing; this layer owns HTTP
...
/** The exec seam the run route uses (local subprocess or remote engine). */
export interface ExecSeam {
  run(command: string, cwd?: string, timeoutMs?: number): Promise<{ ok: boolean; code: number | null; stdout: string; stderr: string }>
}
...
async function readJsonBody(req: IncomingMessage): Promise<unknown> {
  ...
  for await (const chunk of req) {
    ...
    chunks.push(buffer)
    total += buffer.length
    // Binary writes (office save) ship base64; allow a generous 64 MB so a
 ...
 */
export function registerPanelRoutes(ctx: Context, fs: FsService, git: GitService, exec?: ExecSeam): () => void {
  ...
  const pollGit = async (): Promise<void> => {
    ...
    try {
      await Promise.all([...subscribers].map(async (subscriber) => {
        try {
          // SSH-mode roots have no local git service — skip their poll.
  ...
  const handler = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    switch (pathname) {
      ...
      case '/aionui-panel/exec': {
        if (exec === undefined) {
          json(res, FAIL({ code: 'internal', message: 'exec seam unavailable' }))
          return
        }
        const command = strField(payload, 'command')
        const rawCwd = typeof payload === 'object' && payload !== null
        ...
        try {
          const result = await exec.run(command, cwd, 60_000)
          json(res, OK(result))
        } catch (error: unknown) {
          json(res, FAIL({ code: 'internal', message: error instanceof Error ? error.message : String(error) }))
        }
        return
      }
      case '/aionui-panel/git-status': {
        // Over SSH the git service is unavailable: report "not a repository"
        // so the SCM tab degrades cleanly instead of erroring.
        if (fs.isRemote(root)) {
          json(res, OK(null))
          return
        }
      ...
      case '/aionui-panel/git-diff': {
        if (fs.isRemote(root)) {
          json(res, FAIL({ code: 'git-unavailable', message: 'git is unavailable over SSH' }))
          return
        }
        const path = strField(payload, 'path')
        if (path === null) {
  ...
  const sse = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const disposeWatch = fs.watch(gated.canonical, (rel) => {
      push(subscriber, { kind: 'fs', path: rel })
    })
    req.on('close', () => {
      disposeWatch()
