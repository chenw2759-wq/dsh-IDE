...
| 持久连接池 | 每台主机复用长连接（对应 ssh-skill 的守护进程），空闲 30 分钟自动断开，断线自动重连（最多 3 次）；每个连接缓存复用单个 SFTP channel（`client.sftp()` 每次会新开 subsystem channel，不缓存会撞 OpenSSH `MaxSessions` 上限导致远程目录列举/读取间歇性失败）；symlink 按批并行 stat（conda/venv `bin` 目录几十上百个链接不会串行卡死）；ls/stat/readFile 带超时（15s/15s/60s），网络半死时文件树报错返回而不会无限转圈 |
