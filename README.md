...
- 预览标签可**拖拽排序**（拖动 tab 到目标位置）
- 布局四态循环：下框（⇊）/ 右侧弹出（⇉）/ 浮动（⇱）/ **三栏 IDE（⿻）**——对话 | 文件树 | 预览三栏并排
- **预览自由拖拽**：side 布局下拖动预览列宽度把手（0~½ 界面宽，变大即压缩对话、永不覆盖）；float 布局下按住预览标签栏可把浮动窗拖到任意位置（位置记忆 + 180ms 缓动动画），松手时自动吸附到预设位置——**最右 / 盖住文件树（树自动折叠为圆形按钮）/ 树下方 / 聊天区下方**（拖出吸附区即回到自由浮动）
- **聚焦模式**：预览打开时点文件栏「>_」旁的折叠箭头，文件树收为约 30px 右栏 rail（仅留小圆角展开按钮），点击即展开，专注预览与编辑；折叠后**预览框保持原宽度不受影响**
...
- **docx**：段落 / 表格 / 加粗 / 斜体 / 下划线 / 文字颜色 / 字号 / 高亮 / 对齐直接渲染
...
并以二进制写回（mtime 冲突保护）。已知限制：复杂排版（页眉页脚 / 合并单元格等）不保留。

### 🎨 HTML / Markdown 可视化编辑（Word 式原地编辑）

预览工具栏「可视化编辑」：渲染结果本身就是可编辑文档（不再是浮动文本框）——像 Word 一样在渲染页上直接改。**Markdown** 在可编辑区里改编译后的 HTML，保存时转回 markdown 源（标题含中划线不会乱码、代码围栏保留语言）；**HTML** 在完整文档 iframe 里编辑，原 `<style>` 与画布背景照常渲染，保存回写**整个文档**（不再丢成单列覆盖的 body 碎片）。工具栏提供加粗/斜体/下划线/文字颜色/高亮/字体/字号/对齐/撤销/重做。
